...
- **Global Tooling (`~/.omp/agent/mcp.json`)**: Contains core servers baked into the image (`git`, `fetch`, `time`, `docker`, `puppeteer`).
- **Project Tooling (`.omp/mcp.json`)**: Contains stack-specific servers (e.g., `aspire`, `shadcn`) generated by `bootstrap.sh`.

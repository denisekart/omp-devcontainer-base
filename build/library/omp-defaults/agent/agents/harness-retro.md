...
1. If the task does NOT contain a HARNESS FINDINGS document: call `harness_report` (days=7). If it DOES (base-repo intake): the document's proposals are the primary evidence; skip local telemetry unless the document is thin.

---
# base: bundled pi-coding-agent 18.1.10 prompts/agents/reviewer.md
# sha256: c6abab43dcf55776830225a8a8b277fc39b9e3bbdc49271635c28e309ec9c2c0
name: reviewer
      ...
      elements:
        properties:
          ...
          line_end:
            ...
            type: number
---
autoloadSkills: [dotnet-core, test-quality]
...
<project-conventions>
- **Stack**: .NET 10 / ASP.NET Core minimal APIs / EF Core / Aspire AppHost; Svelte 5 + Tailwind + shadcn/ui frontend; xUnit + Playwright .NET tests.
- **CRAP-aware**: flag untested high-complexity paths on modified files (see `test-quality` skill).
- **Severity**: P0 = release blocker/data corruption/auth bypass; P1 = race condition under load; P2 = edge case; P3 = info.
- **Conventions**: Clean Architecture layering, `MapGroup` route conventions, `ProblemDetails` error handling.
- **Aspire workers**: verify cross-boundary event/message consumption — read consumer dispatch point before concluding producer correct.
</project-conventions>

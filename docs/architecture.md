...
3. **`install-omp-plugins.sh`** — installs the 4 pinned plugins via `omp plugin install` as root (the global bun prefix is root-owned; `sudo` passes `HOME`/`BUN_INSTALL` through and user trees are chowned back). Sentinel `~/.omp/.plugins-installed-v1`; lockfile `~/.omp/plugins.lock.json`; degrades to a global npm install if the `omp plugin` subcommand is missing.

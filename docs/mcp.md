...
MCP follows the same profile rules as the rest of OMP-native config; see the [Profiles](#profiles) section above.
...
The `omp-devcontainer-base` image ships two MCP layers.

### User-level (`~/.omp/agent/mcp.json`)

Seeded from the immutable image default (`/usr/local/share/omp-defaults/agent/mcp.json`) on first boot:

| Server | Command |
|--------|---------|
| `git` | `mcp-server-git` (uv-installed) |
| `fetch` | `mcp-server-fetch` (uv-installed) |
| `time` | `mcp-server-time` (uv-installed) |
| `docker` | `mcp-server-docker` (uv-installed) |
| `puppeteer` | `npx -y @modelcontextprotocol/server-puppeteer` |

### Project-level (`.omp/mcp.json`)

Generated by `bootstrap.sh` at first boot, per detected stack:

- `dotnet-aspire-svelte` / `dotnet-only` / `generic` → `docker`, `aspire` (`aspire agent mcp`), `shadcn` (`npx -y shadcn@latest mcp`), `puppeteer`
- `svelte-only` → `shadcn`, `puppeteer`

### Precedence

- **Project > user**: if a server name exists in both the project and the user config, the project-level definition wins.
- **No merging**: same-named servers are NOT merged — the other definition is ignored for that name.
- **Independence**: differently-named servers from both files combine, so you never need to redefine the core servers in your project.

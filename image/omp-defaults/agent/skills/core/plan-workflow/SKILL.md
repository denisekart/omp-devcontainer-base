---
name: plan-workflow
description: Plan/refine/execute workflow for oh-my-pi. Intent routing, gap analysis, approval gate, dual high-accuracy review, and .omo/plans artifact contract. Load this to plan any significant change.
---
...
## Phase 2 — Draft (.omo/drafts/<slug>.md)

Write a draft at `.omo/drafts/<slug>.md` with:
...
Spawn a read-only gap-analysis sub-session (using the `metis-equivalent` persona from `image/omp-defaults/agent/personas/plan-personality.md`) against the draft. Fold findings before writing the plan.

```
pi task --plan "TASK: act as a pre-planning gap analyst (Metis).
Read the draft at .omo/drafts/<slug>.md.
...
## Phase 5 — Plan File (.omo/plans/<slug>.md)

Only after approval:

1. Create `.omo/plans/<slug>.md` with this exact structure:
```
# <slug> - Work Plan
## TL;DR (For humans)
## Scope
...
- `.omo/specs/<slug>/requirements.md` — EARS-style requirements (WHEN/THE/SHALL/SHALL NOT/IF)
- `.omo/specs/<slug>/design.md` — architecture + correctness properties

The `## Todos` section of the plan IS the tasks document — no separate `tasks.md` needed.

See `.omo/specs/_example/` for a complete worked example.
See `.omo/specs/_example_not_triggered/rubric-check.md` for a sub-threshold example where SDD-lite was correctly NOT triggered.

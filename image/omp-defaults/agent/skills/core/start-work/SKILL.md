---
name: start-work
description: Execute an approved .omo/plans/<slug>.md. ONLY invoked explicitly by the user — never by the plan-workflow skill itself. Dispatches implementation subagents, verifies each todo, marks progress.
---

# Start Work

This skill executes an approved plan from `.omo/plans/<slug>.md`.
...
Read `.omo/plans/` and select the target plan. If ambiguous, ask once.
